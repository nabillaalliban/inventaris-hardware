...
<div class="loans-page">
  <section class="loans-hero">
    <div class="loans-hero__inner">
      ...
      <div class="loans-hero__actions">
        <a class="loans-stat-btn" href="{{ route('admin.loans.dashboard') }}">📊 Statistik</a>
      </div>
    </div>
  </section>

  <div class="loans-filters">
    <a class="loans-filter {{ request('status') === 'pending' ? 'is-active' : '' }}" href="{{ route('admin.loans.index',['status'=>'pending']) }}">Pending</a>
    <a class="loans-filter {{ request('status') === 'approved' ? 'is-active' : '' }}" href="{{ route('admin.loans.index',['status'=>'approved']) }}">Aktif</a>
    <a class="loans-filter {{ request('status') === 'overdue' ? 'is-active' : '' }}" href="{{ route('admin.loans.index',['status'=>'overdue']) }}">Jatuh Tempo</a>
    <a class="loans-filter {{ request('status') === 'returned' ? 'is-active' : '' }}" href="{{ route('admin.loans.index',['status'=>'returned']) }}">Dikembalikan</a>
    <a class="loans-filter {{ request('status') === 'rejected' ? 'is-active' : '' }}" href="{{ route('admin.loans.index',['status'=>'rejected']) }}">Ditolak</a>
  </div>

  @if(session('success'))
    <div class="loans-alert loans-alert--success">{{ session('success') }}</div>
  @endif
  ...
  <section class="loans-table-shell">
    <div class="loans-table-head">
      ...
      <div class="loans-table-count">{{ $loans->count() }} Data</div>
    </div>
    ...
    <div class="table-wrap">
      <table class="table">
        ...
        @forelse($loans as $l)
          ...
          <tr>
            <td><span class="loans-index">{{ $loop->iteration }}</span></td>
            <td>
            ...
            <td class="loans-type">{{ strtoupper($l->tipe_peminjam) }}</td>
            <td class="loans-date">{{ $l->tanggal_pinjam }}</td>
            <td class="loans-date">{{ $l->due_date ?? '-' }}</td>

            <td>
              @if($isOverdue)
                <span class="loans-status overdue">overdue</span>
              @else
                <span class="loans-status {{ $l->status }}">{{ $l->status }}</span>
              @endif
            </td>
              ...
              <div class="loans-items">
                @foreach($l->items as $it)
                  <div class="loans-item-row">
                    ...
                    <span class="loans-qty">x{{ $it->qty }}</span>
                  </div>
                @endforeach
              </div>
            </td>
              ...
              <div class="loans-actions__group">
                @if($l->status === 'pending')
                  <div class="loans-actions__inline">
                    <form action="{{ route('admin.loans.approve',$l->id) }}" method="POST">
                      @csrf
                    ...
                    <form action="{{ route('admin.loans.reject',$l->id) }}" method="POST">
                      @csrf
                ...
                @if($l->status === 'approved')
                  <form action="{{ route('admin.loans.returned',$l->id) }}" method="POST" class="loans-return-form">
                    @csrf
                    @method('PUT')
