...
<div class="loan-wrap">
  <div class="loan-card">
    <div class="loan-header">
      ...
      <a class="btn-back" href="{{ route('user.peminjaman.index') }}">
        ← Kembali
      </a>
    </div>
      ...
      <form action="{{ route('user.peminjaman.store') }}" method="POST">
        ...
        <div class="form-grid">
          ...
          <div class="form-group">
            ...
            <input type="date" name="tanggal_pinjam" class="control" value="{{ old('tanggal_pinjam') }}" required>
          </div>

          {{-- Nama Peminjam --}}
          <div class="form-group span-2">
            <div class="label">Nama Peminjam</div>
            <input type="text" name="nama_peminjam" class="control" value="{{ old('nama_peminjam') }}" required>
          </div>
              ...
              >
                ...
                @forelse($inventaris as $i)
                  <option value="{{ $i->id }}" {{ old('inventaris_id')==$i->id ? 'selected' : '' }}>
                    {{ $i->nama_perangkat }} ({{ $i->category->nama_kategori ?? '-' }})
                  </option>
                @empty
                  {{-- Tidak ada item --}}
                @endforelse
          ...
          <div class="form-group span-2">
            ...
            <textarea name="keterangan" class="control" rows="5" placeholder="Tambahkan keterangan jika perlu...">{{ old('keterangan') }}</textarea>
          </div>
        </div>

        <div class="actions">
          <a href="{{ route('user.peminjaman.index') }}" class="btn btn-ghost">Batal</a>
...
{{-- set old inventaris_id ke select saat reload --}}
<script>
  (function () {
    const oldId = @json(old('inventaris_id'));
    if (oldId) {
